export async function onRequest(context) {
  ...
  if (!id || !ownerId) return new Response(JSON.stringify({ success: false, error: 'id and ownerId are required' }), { status: 400, headers: { 'Content-Type': 'application/json', ...corsHeaders } });
  try {
  ...
  const now = Date.now();
  await env.DB.prepare('INSERT INTO boards (id, title, owner_id, created_at, updated_at, node_count) VALUES (?, ?, ?, ?, ?, 0)').bind(id, title || '未命名畫布', ownerId, now, now).run();
  const board = await env.DB.prepare('SELECT * FROM boards WHERE id = ?').bind(id).first();
  return new Response(JSON.stringify({ success: true, board: { id: board.id, title: board.title, ownerId: board.owner_id, createdAt: board.created_at, updatedAt: board.updated_at, nodeCount: 0 } }), { headers: { 'Content-Type': 'application/json', ...corsHeaders } });
}
