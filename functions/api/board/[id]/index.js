export async function onRequest(context) {
  ...
  if (request.method === 'GET') {
    ...
    return new Response(JSON.stringify({ success: true, board: mapBoard(board) }), {
      headers: { 'Content-Type': 'application/json', ...corsHeaders },
    });
  }
  ...
  if (request.method === 'PATCH') {
    ...
    if (!board || board.owner_id !== ownerId)
      return new Response(
        JSON.stringify({ success: false, error: 'Only the board owner can rename it' }),
        { status: 403, headers: { 'Content-Type': 'application/json', ...corsHeaders } },
      );
    await env.DB.prepare('UPDATE boards SET title = ?, updated_at = ? WHERE id = ?')
      .bind(String(title || '').trim() || board.title, Date.now(), boardId)
      .run();
    const updated = await env.DB.prepare('SELECT * FROM boards WHERE id = ?').bind(boardId).first();
    return new Response(JSON.stringify({ success: true, board: mapBoard(updated) }), {
      headers: { 'Content-Type': 'application/json', ...corsHeaders },
    });
  }
